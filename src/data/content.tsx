...
export const technicalSkills: SkillArticle[] = [
  {
    ...
    objective: (
      <p>La mission consistait à recréer l'interface de la page d'accueil du site ciblé tout en implémentant au moins deux fonctionnalités représentatives du parcours utilisateur. Il s'agissait d'assurer une démonstration fluide, visuellement convaincante et suffisamment représentative des cas d'usage réels. Le tout devait être livré dans un délai extrêmement restreint, avec une exigence élevée sur la qualité visuelle.</p>
    ),
    challenges: (
      <p>L'enjeu principal était d'ordre stratégique. Il fallait présenter SAP Fiori sous son meilleur jour, malgré une maîtrise encore incomplète de la technologie. Le livrable devait susciter l'adhésion du client et incarner la promesse d'une transformation digitale réussie via SAP, sans laisser entrevoir les limitations techniques du prototype.</p>
    ),
    risks: (
      <p>Le principal risque tenait à mon manque d'expérience sur SAP UI5 au moment de la mission. Un autre point de vigilance concernait la gestion du stress et la capacité à livrer un résultat professionnel dans un contexte de forte pression. Enfin, une tentative de démonstration trop ambitieuse aurait pu fragiliser l'ensemble du projet si certaines parties de l'interface s'étaient révélées instables ou incomplètes.</p>
    ),
    presentation: (
      <p>SAP Fiori repose sur le framework UI5, fortement inspiré de JavaScript et structuré selon le modèle MVC. L'interface est composée de "controls", des widgets que l'on assemble pour créer des vues dynamiques. L'apprentissage de l'outil est facilité par la documentation SAP, qui propose de nombreux exemples visuels et des fragments de code exploitables directement. Dans ce projet, j'ai structuré mon travail autour d'une modélisation rigoureuse du site d'origine, d'un travail sur l'esthétique de l'interface, puis de l'implémentation des règles de calcul métier nécessaires à la simulation.</p>
    ),
    explanation: (
      <p>Un développeur web ayant déjà pratiqué JavaScript peut rapidement s'orienter dans l'écosystème Fiori/UI5, bien que des efforts d'adaptation soient nécessaires pour comprendre les spécificités de la structure MVC imposée et le fonctionnement précis des composants SAP. La documentation officielle joue ici un rôle clé pour guider la progression et aider à comprendre les bonnes pratiques d'assemblage, de binding et de logique applicative.</p>
    ),
    results: (
      <>
        <p>La maquette a été livrée dans les délais, sans retour négatif lors de la démonstration client. J'ai pu implémenter les fonctionnalités critiques et même améliorer certains aspects graphiques du site original, ce qui a contribué à renforcer l'impact de la démonstration. Cette expérience m'a permis de découvrir en conditions réelles les bases de SAP UI5, tout en mettant en avant mes capacités à travailler sous pression et à livrer un résultat valorisant pour l'entreprise.</p>
        <p><strong>Maîtrise, progression et recul:</strong> Je dispose aujourd'hui d'une maîtrise opérationnelle de la conception de maquettes SAP Fiori à des fins de démonstration. Néanmoins, la logique complète du framework UI5, notamment pour ce qui touche aux intégrations avec des services back-end ou aux composants avancés, reste à approfondir. Cette compétence s'inscrit de manière stratégique dans mon profil, notamment pour des rôles de consultant ou d'ingénieur avant-vente SAP.</p>
        <p>Mon apprentissage s'est fait rapidement sous contrainte, mais sans me permettre d'explorer en profondeur toutes les possibilités offertes par la technologie. Avec le recul, je retiens qu'en contexte d'avant-vente, il est préférable de cibler quelques fonctionnalités clés et de les exécuter parfaitement plutôt que de viser une couverture large mais instable.</p>
